...
class FusionThresholds:
    # Physiology probability thresholds (now calibrated)
    ...
    # Trend threshold: if stress probability rises by >= this amount over the last K windows
...
def decide_action(
    ...
    """
    Explainable fusion: combine physiology + air context into an action recommendation.

    Returns a dict with:
      - status: overall state label
      - action: what the system suggests doing
    ...
    # We keep it rule-based for explainability.
    ...
    if stress_level == RiskLevel.MEDIUM and air_level == RiskLevel.HIGH:
        return {
            ...
            "rationale": "Moderate physiological strain + high air risk suggests exposure-driven risk; context-aware adjustment reduces load.",
            "stress_level": stress_level.value,
            "air_level": air_level.value,
        }
    ...
    if rising and stress_level != RiskLevel.HIGH:
        return {
            ...
            "rationale": f"Stress probability is rising (Δ≈{trend:.2f}) even if not yet high; early intervention prevents deterioration.",
            "stress_level": stress_level.value,
            "air_level": air_level.value,
        }
