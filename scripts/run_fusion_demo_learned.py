...
from src.state_postprocess import SpecialStateConfig, enforce_special_states


def main() -> None:
    # Load WESAD window features
    ...
    # Load strict physiology bundle
    phys_path = PROJECT_ROOT / "outputs" / "models" / "physio_rf_strict_calibrated_bundle.joblib"
    if not phys_path.exists():
        raise FileNotFoundError("Missing strict physio bundle. Run train_physio_model_strict.py.")
    phys_bundle = joblib.load(phys_path)

    # Compute calibrated stress likelihood
    ...
    # Load learned fusion model
    fusion_path = PROJECT_ROOT / "outputs" / "models" / "fusion_decision_tree.joblib"
    if not fusion_path.exists():
        raise FileNotFoundError("Missing fusion tree. Run train_fusion_model.py first.")
    ...
    # Build fusion features using smoothed signals (important)
    tmp = df_feat.copy()
    ...
    # --- thresholds from physio bundle (calibrated) ---
    t_med = float(phys_bundle.get("threshold_f1", 0.24))
    t_high = float(min(0.95, t_med + 0.15))

    # status_raw from tree already computed
    status_raw = tree.predict(X)
    df_feat["status_raw"] = status_raw

    # Enforce special states based on smoothed signals + calibrated thresholds
    ...
    enforced = enforce_special_states(
    ...
    df_feat["status_enforced"] = enforced

    # Now smooth the enforced statuses (not the raw ones)
    df_feat["status"] = hysteresis_smooth_status(df_feat["status_enforced"].astype(str).tolist(), smooth_cfg)

    # Attach actions/rationales (based on final smoothed status)
    ...
    # Compare raw vs smoothed flips (simple indicator)
    raw = df_feat["status_raw"].astype(str).to_list()
    sm = df_feat["status"].astype(str).to_list()
    raw_flips = sum(raw[i] != raw[i - 1] for i in range(1, len(raw)))
    sm_flips = sum(sm[i] != sm[i - 1] for i in range(1, len(sm)))
    print(f"\nFlip count: raw={raw_flips} vs smoothed={sm_flips} (lower is better)")
